...
const JobSection = ({ preference }) => {
  const { state } = useAppContextValue();
  // console.log(useAppContextValue());

  const jobType = [...state.filterArray].reduce((acc, value) => {
  ...
  const jobLocation = [...state.filterArray].reduce((acc, value) => {
  ...
  return (
    <Grid as="section" gridTemplateColumns={preference ? "2fr 3fr" : "1fr 2fr 3fr"} gap={10}>
      {/* <GridItem w="100%" bgColor="#1c1c24" borderRadius="10" p={5}> */}

      {!preference && (
        <GridItem w="100%">
          <Filter list={typeArray} type={true} heading="Type of Employment" />
          <Filter list={locationArray} type={false} heading="Job Location" />
        </GridItem>
      )}

      <GridItem w="100%">
        <JobContainer preference={true} />
      </GridItem>

      <GridItem w="100%" maxW="100%">
        <JobInfoContainer preference={false} />
      </GridItem>
    </Grid>
  );
};
